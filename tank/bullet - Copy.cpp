...
bullet::bullet(int hid,int pbid,int px,int py,int pd,void *pp[60],int ps=10){
               #ifdef debug_funtions 
               cout<<"bullet::const"<<endl;
               #endif
               hostId=hid;
               objptr=pp[3];
               ...
               step=ps;
               enabled=1;
               //getimage(8000,670,800+AW_W,670+AW_H,data);
...
void bullet::procedure(){
     ...
     if(enabled){
                         ...
                         putimage(x,y,data,COPY_PUT);
                          ...
                          if (x>=AREA_WIDTH||y>=AREA_HEIGHT||x<-AW_W/2||y<-AW_H/2)
                          enabled=0;
                          else{
    						  if (dir==0)
                              if(check(x,y-step)){
                                   ...
                                   if ((int)((machine *)(*((int *)objptr+target)))->id/10!=(int)hostId/10)
                                     ((machine *)(*((int *)objptr+target)))->die();
                                     if (target>10) {
                                                        *((int *)(result+(hostId-1)*5+(target-10)))=*((int *)(result+(hostId-1)*5+(target-10)))+1;
                                                        //hostId=1
                                                        //target=11
                                                        //res+0+2
